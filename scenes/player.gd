...
const WALL_JUMP_VELOCITY := Vector2(400, -380)
const KNOCHBACK_AMOUNT := 512.0
const SLIDING_DURATION := 0.3
const SLIDE_SPEED := -256.0
const LANDING_HEIGHT := 100.0
const SLIDE_ENERGY := 1.0
...
var interactale_with: Array[Interactable]
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("interact") and interactale_with:
		interactale_with.back().interact()

func tick_physics(state: State, delta: float) -> void:
	interaction_icon.visible = not interactale_with.is_empty()
...
func register_interactable(v: Interactable) -> void:
	if state_machine.current_state == State.IDLE:
		return
	if v in interactale_with:
		return
	interactale_with.append(v)
	
func unregister_interactable(v: Interactable) -> void:
	interactale_with.erase(v)
